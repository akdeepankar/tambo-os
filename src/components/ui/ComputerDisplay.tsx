		...
		<div className="relative w-full h-full">
			<div className="w-full h-full">
				...
				<div style={{ display: activeTab === "globe" ? "block" : "none" }}>
					<Globe lat={globeResult?.lat} lng={globeResult?.lng} address={globeResult?.address} routePolyline={globeResult?.routePolyline} />
				</div>
				<div style={{ display: activeTab === "calendar" ? "block" : "none" }}>
					<Calendar />
				</div>
				{/* Render children for other tabs */}
				{activeTab !== "notepad" && activeTab !== "globe" && activeTab !== "dictionary" && activeTab !== "calendar" ? children : null}
			</div>
			<div className="absolute left-1/2 bottom-6 -translate-x-1/2 z-50">
				<FloatingDock
					items={[
						...
						{
							title: "Photo Editor",
							icon: <IconPaint className="h-full w-full text-neutral-500 dark:text-neutral-300" />,
							href: "#photo",
							key: "photo",
						},
						{
						...
						{
							title: "Maps",
							icon: <IconWorld className="h-full w-full text-neutral-500 dark:text-neutral-300" />,
							href: "#globe",
							key: "globe",
						},
						{
