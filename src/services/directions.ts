...
export async function directionsTool({ origin, destination, travelMode = "DRIVING" }: DirectionsInput): Promise<DirectionsResult> {
  ...
  if (!(window as Window & typeof globalThis & { google?: typeof google }).google) throw new Error("Google Maps JS API not loaded");

  // Get place IDs for origin and destination
  function getPlaceId(query: string): Promise<string> {
    return new Promise((resolve, reject) => {
      const service = new ((window as Window & typeof globalThis & { google?: typeof google }).google.maps.places.PlacesService)(document.createElement("div"));
      service.textSearch({ query }, (results: google.maps.places.PlaceResult[] | null, status: google.maps.places.PlacesServiceStatus, pagination: google.maps.places.PlaceSearchPagination | null) => {
        if (status === ((window as Window & typeof globalThis & { google?: typeof google }).google.maps.places.PlacesServiceStatus.OK) && results && results[0]) {
  ...
  // Request directions
  return new Promise((resolve, reject) => {
    const directionsService = new ((window as Window & typeof globalThis & { google?: typeof google }).google.maps.DirectionsService)();
    directionsService.route({
      ...
      if (status === "OK" && result && result.routes && result.routes[0]) {
        ...
        if (typeof window !== "undefined") {
          (window as Window & typeof globalThis & { _lastDirectionsLegs?: Array<google.maps.DirectionsLeg> })._lastDirectionsLegs = legs;
        }
        ...
        const win = window as Window & typeof globalThis & { handlePlaceSearchResult?: (result: { lat: number; lng: number; address: string; routePolyline?: string }) => void };
        if (typeof window !== "undefined" && typeof win.handlePlaceSearchResult === "function") {
          win.handlePlaceSearchResult(globeResult);
        }
